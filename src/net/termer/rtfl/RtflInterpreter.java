...
public class RtflInterpreter {
	...
	public void closeTerminalInput() throws RtflException {
		if(TERM!=null) {
			try {
				...
				throw new RtflException("failed to close terminal input: "+e.getMessage());
			}
		}
	}
	...
	public String readTerminalInput() throws RtflException {
		String line = null;
		if(TERM==null) {
			throw new RtflException("terminal input is not open");
		} else {
			try {
				line = TERM.readLine();
			} catch (IOException e) {
				throw new RtflException("failed to read terminal input: "+e.getMessage());
			}
		}
		return line;
	}
	 ...
	 * @since 1.0
	 */
	public Object execute(String rtfl) {
		String[] lines = null;
		Object result = null;
		if(rtfl.contains("\n")) {
			lines = rtfl.split("\n");
			...
			try {
				if(!line.startsWith("//") && !line.isEmpty()) {
					if(line.startsWith("goto ")) {
						index = EXP.getNumberValue(line.substring(5)).intValue();
						...
						if(line.contains("=")) {
							...
							if(Text.onlyContains(Text.LETTERS_NUMBERS_AND_UNDERSCORES, name)) {
								if(name.startsWith("arg")) {
									throw new RtflException("you cannot begin variable names with \"arg\"");
								} else {
									if(Text.isNumber(name.charAt(0))) {
										throw new RtflException("variable names may not start with numbers");
									} else {
										String exp = line.substring(line.indexOf('=')+1);
										VARS.register(name, EXP.getValue(exp));
									}
								}
							} else {
								throw new RtflException("variable names may only contain letters, numbers, and underscores");
							}
						} else {
							throw new RtflException("no value specified");
						}
						...
						if(Text.onlyContains(Text.LETTERS_NUMBERS_AND_UNDERSCORES, name)) {
							if(index<lines.length-2) {
								...
								throw new RtflException("unclosed curly bracket");
							}
						} else {
							throw new RtflException("function names may only contain letters, numbers, and underscores");
						}
					} else if(line.startsWith("unfunc ")) {
						FUNC.clear(line.substring(7));
					} else if(line.startsWith("return ")) {
						// Sets the return value;
						result = EXP.getValue(line.substring(7));
						...
						if(index<lines.length-2) {
							...
							Object logic = EXP.getValue(exp);
							if(logic instanceof Boolean) {
								if(((Boolean)logic).booleanValue()) {
									execute(body);
								}
							} else {
								throw new RtflException("expression in if statement does not return a boolean");
							}
						} else {
							throw new RtflException("unclosed curly bracket");
						}
						...
						if(line.contains("=")) {
							if(!Text.isInQuotes(line, line.indexOf('='), false)) {
								...
								if(EXP.isVar(name)) {
									...
									throw new RtflException("referenced variable \""+name+"\" is undefined");
								}
							}
						}
						if(varSet) {
							VARS.set(line.split("=")[0].trim(), EXP.getValue(line.substring(line.indexOf('=')+1).trim()));
						} else if(EXP.isFunc(line)) {
							EXP.getFuncValue(line);
						} else {
							throw new RtflException("undefined function");
						}
					}
				}
			} catch(RtflException e) {
				System.out.println("Error on line "+Integer.toString(index+1)+": "+e.getMessage());
			}
			index++;
		}
